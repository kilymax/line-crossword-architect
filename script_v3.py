...
class Main(Tk):
    ...
    def __init__(self):
        ...
        # 2 поле конфигурации
        self.word_count_label = ttk.Label(self.leftframe, background=self.majorcolor,  justify=LEFT,
                                   foreground=self.labelfgcolor, font=self.font10, text='Кол-во строк:')
        self.word_count_label.grid(row=8, column=0, padx=10, pady=3, sticky="w")
        self.entry2 = Entry(self.leftframe, justify=CENTER, width=15)
        self.entry2.bind('<FocusOut>', self.makeEmptyPhraseList)
        self.entry2.grid(row=8, column=1, columnspan=3, padx=10, pady=3, sticky="w")
        ...
        self.bd_size_label.grid(row=9, column=0, columnspan=2, padx=10, sticky="w")
        ...
        self.bd_scale.set(1)
        self.bd_scale.grid(row=9, column=1, columnspan=2, padx=10, pady=0, sticky="nsew")
        ...
        self.font_label.grid(row=10, column=0, padx=10, sticky="w")
        # self.font_var = StringVar(value=font.families()[11])  
        self.font_combobox = ttk.Combobox(self.leftframe, values=font.families(), width=12, state="readonly")
        self.font_combobox.set(font.families()[11])
        self.font_combobox.grid(row=10, column=1, columnspan=2, padx=10, pady=3, sticky="w")
        ...
        self.def_cell_color_label.grid(row=11, column=0, padx=10, sticky="w")
        self.select_color1_btn = Button(self.leftframe, relief="groove", justify=CENTER, font=self.font9, width=4, command = lambda: self.choose_color(1))
        self.select_color1_btn.grid(row=11, column=1, columnspan=1, padx=10, pady=3, sticky="nsew")
        self.select_color1_btn.configure(state='normal', fg="#b8b8b8", bg=self.cellcolor)

        self.select_fontcolor1_btn = Button(self.leftframe, relief="groove", justify=CENTER, font=self.font9, width=4, command = lambda: self.choose_color(3))
        self.select_fontcolor1_btn.grid(row=11, column=2, columnspan=1, padx=10, pady=3, sticky="nsew")
        ...
        self.fixed_color_label.grid(row=12, column=0, padx=10, sticky="w")
        self.select_color2_btn = Button(self.leftframe, relief="groove", justify=CENTER, font=self.font9, width=4, command = lambda: self.choose_color(2))
        self.select_color2_btn.grid(row=12, column=1, columnspan=1, padx=10, pady=3, sticky="nsew")
        self.select_color2_btn.configure(state='normal', fg="#b8b8b8", bg=self.fixedcellcolor)

        self.select_fontcolor2_btn = Button(self.leftframe, relief="groove", justify=CENTER, font=self.font9, width=4, command = lambda: self.choose_color(4))
        self.select_fontcolor2_btn.grid(row=12, column=2, columnspan=1, padx=10, pady=3, sticky="nsew")
        ...
        self.gridbutton.grid(row=13, column=0, columnspan=4, pady=3, padx=10, sticky="nsew")
        ...
        self.maxlabel = ttk.Label(self.leftframe, style="infolabel.TLabel", 
                                   background=self.majorcolor, font=self.font10, text='Смешать')
        ...
        self.left_shift_button.grid(row=18, column=1, columnspan=1, pady=3, padx=10, sticky="ew")
        
        self.right_shift_button = Button(self.leftframe, command=self.right_shift_fixed_cells, text="→", font=self.font13)
        self.right_shift_button.grid(row=18, column=2, columnspan=1, pady=3, padx=10, sticky="ew")
    ...
    def choose_color(self, num):
        ...
        if num == 1:
            self.cellcolor = colorchooser.askcolor(title ="Выберите цвет")[1]
            self.select_color1_btn.configure(bg=self.cellcolor)
        if num == 2:
            self.fixedcellcolor = colorchooser.askcolor(title ="Выберите цвет")[1]
            self.select_color2_btn.configure(bg=self.fixedcellcolor)
        if num == 3:
            self.font_color1 = colorchooser.askcolor(title ="Выберите цвет")[1]
            self.select_fontcolor1_btn.configure(bg=self.font_color1)
        if num == 4:
            self.font_color2 = colorchooser.askcolor(title ="Выберите цвет")[1]
    ...
    def makeEmptyPhraseList(self, event):
        entryInput = event.widget.get()
        print(entryInput.isdigit())
        if entryInput.isdigit():
            self.h = int(entryInput)
            self.fixed_phrase = ['']*self.h
            print(f'fixed_phrase: {self.fixed_phrase}')
    ...
    def fixing_cell(self, event, entry, i, j):
        if event.char in self.global_alphabet:
            ...
            if event.char in self.tatar_extension:
                ...
                self.fixed_phrase[i] = self.tatar_extension[event.char].upper()
            else:
                entry.insert(0, event.char.upper())
                self.fixed_phrase[i] = event.char.upper()
            entry.config(validate="key", fg=self.font_color2, bg=self.fixedcellcolor, 
                        validatecommand=self.char_check)
            # self.
    ...
    def make_crossword_grid(self, w, h):
        ...
        if w.isdigit() and h.isdigit():
            try:
                ...
                self.w = int(w)
                self.h = int(h)
                # self.phrase_position = [-1]*self.w
                self.grid = []
                self.enabledcell = []
                #self.crosswordframe.config(width=self.w*2, height=self.h*5)
                self.fontcoeff = round( 400/max([self.w, self.h]) )
                ...
                for i in range(self.h):
                    ...
                    for j in range(self.w):
                        ...
                        if j == self.phrase_position:
                            self.grid[i][j].configure(state="normal", bg=self.fixedcellcolor)
    ...
    def makeRightSide(self):
        ...
        for i in range(len(new_tails_list)):
            ...
            while len(word) < max_tail_length:
                word+=' '
            # очищаем клетки, если были заполнены
            for j in range(self.w):
                try:
                    print(self.grid[i][self.w+j+2].get(), j)
                    self.grid[i][-1].configure(state="normal")
                except: pass
            for j in range(len(word)):
                tempobj = Entry(self.crosswordframe, justify=CENTER, font=(self.cellfont, self.fontcoeff, 'bold'), fg=self.font_color1, 
                            relief='solid', width=2, validate="key", bg=self.cellcolor, borderwidth=self.bd_scale.get(),
                            validatecommand=self.char_check, disabledbackground=self.disabledentrycolor, readonlybackground=self.cellcolor)
                if j == 0:
                    tempobj.configure(readonlybackground=self.fixedcellcolor)
    ...
    def generator(self):
        ...
        for i in range(self.h):
            ...
            self.img_label = Entry(self.crosswordframe, justify=CENTER, font=(self.cellfont, self.fontcoeff, 'bold'), 
                                relief='solid', width=2, state=DISABLED, 
                                validate="key", disabledbackground=self.disabledentrycolor,)
        ...
        for i in range(self.h):
            for j in range(self.w): 
                self.grid[i][j].configure(state="readonly")
                if j == self.phrase_position:
                    self.grid[i][j].configure(readonlybackground=self.fixedcellcolor)

    # функция сохранения в pdf файл
    def save_in_file(self):
        self.wm_geometry("+%d+%d" % (20, 5))
...
if __name__ == "__main__":
    # print(time.time())
    # if (time.time()) < 1716816043.604615 + 600000: # program blocker
    main = Main()
    main.geometry(f'{1200}x{660}') # main.winfo_screenheight()
    main.wm_geometry("+%d+%d" % (20, 5))
